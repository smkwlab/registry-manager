...
| `review_flow` | boolean | draft PR サイクルで運用するリポジトリか | タイプとは独立した属性。登録時の既定値: sotsuron / master / ise / poster → true、wr / other → false、latex → false（作成時オプトイン）。読み手はタイプからのフォールバック推論をしない | 登録時に決定（`--review-flow` で上書き可） |
