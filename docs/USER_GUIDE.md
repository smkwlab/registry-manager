...
Registry Manager は、学生リポジトリレジストリ(`data/registry.json`)を管理するためのコマンドラインツールです。
...
```bash
# リポジトリを取得
git clone https://github.com/smkwlab/registry-manager.git
cd registry-manager
...
# タイプだけ明示指定（名前に規則がなく推論できない場合。学生IDは推論のまま）
...
卒業済みリポジトリを archive します（open PR クローズ → GitHub リポジトリの archive → `archived_at` 記録）。

```bash
./registry-manager archive REPO_NAME
./registry-manager archive --graduated [--list | --dry-run | -i]
```

**オプション:**
- `--graduated` - 名簿突合で卒業済み学生の登録リポジトリを一括対象にする
- `--list` - 候補一覧を判定理由つきで表示のみ（実行しない）
- `--dry-run` - 副作用なしのシミュレーション
- `-i, --interactive` - 候補を 1 件ずつ確認しながら実行
...
- [データ構造仕様](data-structure-specification.md) - `registry.json` のスキーマ（正本）
- [README](../README.md) - セットアップと概要

### サポート

問題が解決しない場合：

1. [GitHub Issues](https://github.com/smkwlab/registry-manager/issues)
2. プロジェクトドキュメントの確認
3. 管理者への連絡